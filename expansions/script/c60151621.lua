...
function c60151621.filter(c,tp)
	local lv=c:GetLevel()
    return c:IsFaceup() and c:IsSetCard(0xc204) and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_XYZ) 
		and Duel.IsExistingMatchingCard(c60151621.pcfilter,tp,LOCATION_DECK,0,1,nil,lv)
end
function c60151621.pcfilter(c,lv)
    return c:IsSetCard(0xc204) and c:IsType(TYPE_PENDULUM) and not c:IsForbidden() and c:GetLevel()==lv
end
