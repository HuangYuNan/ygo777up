--初之辉
function c23310000.initial_effect(c)
function Auxiliary.AddXyzProcedure(c,f,lv,ct,alterf,desc,maxct,op)
	...
	e1:SetTarget(c23310000.target)
	e1:SetOperation(c23310000.activate)
	c:RegisterEffect(e1)
end
function c23310000.filter0(c,tc)
	return c:IsCanBeXyzMaterial(tc) and tc.xyz_filter(c) and c:IsLocation(LOCATION_MZONE)
end
function c23310000.filter1(c,tc)
	return c:IsCanBeXyzMaterial(tc) and tc.xyz_filter(c)
end
function c23310000.filter2(c,e,tp)
	return c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false)
end
function c23310000.mfilter(c,e,tp,mg)
	return c:IsXyzSummonable(mg) and (Duel.GetLocationCount(e:GetOwner(),LOCATION_MZONE)>0 or mg:IsExists(c23310000.lfilter,1,nil,e,tp,c))
end
function c23310000.lfilter(c,e,tp,mc)
	return c:IsLocation(LOCATION_MZONE) and c:GetControler()==e:GetOwner() and mc.xyz_filter(c)
end
function c23310000.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local mg=Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
	local exg=Duel.GetMatchingGroup(c23310000.filter2,tp,LOCATION_EXTRA,0,nil,e,tp)
	if chk==0 then
		return exg:IsExists(c23310000.mfilter,1,nil,e,tp,mg)
	end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function c23310000.activate(e,tp,eg,ep,ev,re,r,rp)
	local mg1=Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
	local exg=Duel.GetMatchingGroup(c23310000.filter2,tp,LOCATION_EXTRA,0,nil,e,tp)
	local sg=exg:Filter(c23310000.mfilter,nil,e,tp,mg1)
	if sg:GetCount()>0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
			...
			local mg=mg1:Filter(c23310000.filter1,nil,tc):Select(tp,tc.xyz_count,tc.xyz_maxcount,nil)
			Duel.XyzSummon(tp,tc,mg)
		else
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
			local mg0=mg1:Filter(c23310000.filter0,nil,tc):Select(tp,1,1,nil)
			m1:RemoveCard(m0:GetFirst())
			local mg=mg1:Filter(c23310000.filter1,nil,tc):Select(tp,tc.xyz_count-1,tc.xyz_maxcount-1,nil)
			mg:Merge(mg0)
			Duel.XyzSummon(tp,tc,mg)
		end
	end
