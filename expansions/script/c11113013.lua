...
function c11113013.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c11113013.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c11113013.filter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c11113013.filter,tp,LOCATION_REMOVED,0,1,1,nil)
...
function c11113013.thfilter(c)
	return c:IsCode(11113004) and c:IsAbleToHand()
end
