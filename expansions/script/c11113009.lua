...
function c11113009.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c11113009.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c11113009.filter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c11113009.filter,tp,LOCATION_REMOVED,0,1,1,nil)
...
function c11113009.thfilter(c)
	return c:IsCode(11113003) and c:IsAbleToHand()
end
