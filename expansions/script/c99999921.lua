...
function c99999921.spfilter1(c,tp,fc)
	return c:IsRace(RACE_SPELLCASTER) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(fc)
		and Duel.IsExistingMatchingCard(c99999921.spfilter2,tp,LOCATION_MZONE,0,1,c,fc)
end
function c99999921.spfilter2(c,fc)
	return  c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeFusionMaterial(fc) and c:IsAbleToGraveAsCost()
end
