...
function c5012608.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x350) 
end
function c5012608.disfilter(c)
	return aux.disfilter1(c)
end
function c5012608.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c5012608.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) 
	and Duel.IsExistingMatchingCard(c5012608.disfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,nil,1,0,0)
end
function c5012608.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectMatchingCard(tp,c5012608.disfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,e:GetHandler())
	local tc=g:GetFirst()
...
function c5012608.tdcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsReason(REASON_DESTROY) and c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousLocation(LOCATION_SZONE) 
	and c:GetSequence()<5 and c:GetPreviousControler()==tp and rp~=tp
end
...
function c5012608.spfilter(c,e,tp)
	return  c:IsCode(5012604)  and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,false)
end
...
function c5012608.tdop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.GetMatchingGroup(c5012608.tdfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
	if g:GetCount()>0 and Duel.SpecialSummon(g,SUMMON_TYPE_SYNCHRO,tp,tp,false,false,POS_FACEUP)~=0 and  tc:GetCount()>0 then
