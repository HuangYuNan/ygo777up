...
function c80005004.lvtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c80005004.filter,tp,LOCATION_MZONE,0,1,nil) end
end
function c80005004.lvop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c80005004.filter,tp,LOCATION_MZONE,0,nil)
	local tc=g:GetFirst()
	while tc do
		local e1=Effect.CreateEffect(e:GetHandler())
		e1:SetType(EFFECT_TYPE_SINGLE)
...
function c80005004.filter1(c,e,tp)
	return c:IsRace(RACE_SPELLCASTER) and not c:IsType(TYPE_PENDULUM) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:GetAttack()==-2 and c:GetDefense()==-2
end
