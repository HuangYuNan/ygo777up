...
function c80000028.initial_effect(c)
	...
	e2:SetCondition(c80000028.handcon)
...
function c80000028.handfilter(c)
	return c:IsCode(80000028)
end
function c80000028.handcon(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetFieldGroup(e:GetHandlerPlayer(),LOCATION_DECK+LOCATION_GRAVE+LOCATION_HAND,0)
	return not g:IsExists(c80000028.handfilter,1,nil)
end
...
function c80000028.desrepcon(e)
	local g=Duel.GetFieldGroup(e:GetHandlerPlayer(),LOCATION_DECK+LOCATION_GRAVE+LOCATION_HAND,0)
	return not g:IsExists(c80000028.handfilter,1,nil) and Duel.IsExistingMatchingCard(c80000028.filter1,e:GetHandler():GetControler(),LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
end
...
function c80000028.condition(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetFieldGroup(tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_HAND,0):FilterCount(Card.IsCode,nil,e:GetHandler():GetCode())~=0 then return false end
	local lv=0
