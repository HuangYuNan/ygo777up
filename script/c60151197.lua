...
function c60151197.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		if Duel.SendtoGrave(g,REASON_EFFECT) then
			local g3=Duel.GetMatchingGroup(Card.IsAbleToGrave,1-tp,LOCATION_MZONE+LOCATION_HAND,0,nil,e:GetHandler())
			if g3:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(60151197,0)) then Duel.BreakEffect()
				local g1=Duel.SelectMatchingCard(tp,c60151197.filter2,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,e:GetHandler())
