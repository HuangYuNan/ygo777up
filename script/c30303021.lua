--魔人少女 色欲之露西莉亚
function c30303021.initial_effect(c)
	--synchro summon
	...
	e1:SetCondition(c30303021.remcon)
	--e1:SetCost(c30303021.cost)
	e1:SetTarget(c30303021.remtg2)
	e1:SetOperation(c30303021.remop2)
	c:RegisterEffect(e1)
	...
	--e3:SetValue(c30303021.efilter)
	--c:RegisterEffect(e3)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e2:SetValue(c30303021.ind2)
	c:RegisterEffect(e2)
end
function c30303021.remcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c30303021.costfilter(c)
	return c:IsSetCard(0xabb) and c:IsAbleToRemoveAsCost()
end
function c30303021.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(30303021)
end
function c30303021.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c30303021.costfilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,nil) end
	if Duel.IsExistingTarget(c30303021.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
	local g=Duel.SelectTarget(tp,c30303021.costfilter,tp,LOCATION_ONFIELD+LOCATION_HAND+LOCATION_GRAVE,0,1,3,nil)
	ct=Duel.Remove(g,POS_FACEUP,REASON_COST)
	e:SetLabel(ct)
	else
	local g=Duel.SelectTarget(tp,c30303021.costfilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,3,nil)
	ct=Duel.Remove(g,POS_FACEUP,REASON_COST)
	e:SetLabel(ct)
	end
end
function c30303021.remfilter1(c)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and c:IsFaceup()
end
function c30303021.remfilter2(c)
	return c:IsType(TYPE_SPELL) and c:IsAbleToRemoveAsCost() and c:IsFaceup()
end
function c30303021.remfilter3(c)
	return c:IsType(TYPE_TRAP) and c:IsAbleToRemoveAsCost() and c:IsFaceup()
end
function c30303021.remtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c30303021.remfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsExistingMatchingCard(c30303021.remfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsExistingMatchingCard(c30303021.remfilter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,0,0x1e)
end
function c30303021.remop2(e,tp,eg,ep,ev,re,r,rp)
	local g1=Duel.GetMatchingGroup(c30303021.remfilter1,tp,0,LOCATION_ONFIELD,nil)
	local g2=Duel.GetMatchingGroup(c30303021.remfilter2,tp,0,LOCATION_ONFIELD,nil)
	local g3=Duel.GetMatchingGroup(c30303021.remfilter3,tp,0,LOCATION_ONFIELD,nil)
	local sg=Group.CreateGroup()
	if g1:GetCount()>0 and ((g2:GetCount()==0 and g3:GetCount()==0) or Duel.SelectYesNo(tp,aux.Stringid(30303021,0))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local sg1=g1:Select(tp,1,1,nil)
		Duel.HintSelection(sg1)
		sg:Merge(sg1)
	end
	if g2:GetCount()>0 and ((sg:GetCount()==0 and g3:GetCount()==0) or Duel.SelectYesNo(tp,aux.Stringid(30303021,1))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local sg2=g2:Select(tp,1,1,nil)
		Duel.HintSelection(sg2)
		sg:Merge(sg2)
	end
	if g3:GetCount()>0 and (sg:GetCount()==0 or Duel.SelectYesNo(tp,aux.Stringid(30303021,2))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
...
function c30303021.remtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c30303021.efilter(e,re)
