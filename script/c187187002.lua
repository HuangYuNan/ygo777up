...
function c187187002.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c187187002.filter,tp,LOCATION_GRAVE+LOCATION_EXTRA+LOCATION_HAND,0,1,nil) end
...
function c187187002.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c187187002.filter,tp,LOCATION_GRAVE+LOCATION_EXTRA+LOCATION_HAND,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
...
function c187187002.spcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c187187002.filter,c:GetControler(),LOCATION_GRAVE+LOCATION_EXTRA,0,nil)
...
function c187187002.spfilter(c,e,tp)
	return c:IsCode(187187003) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,true,true)
end
