...
function c18723404.filter(c)
	return c:IsAbleToHand() and c:IsSetCard(0xbf) and not c:IsCode(18723404)
end
function c18723404.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c18723404.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c18723404.op1(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c18723404.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c18723404.distarget(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c18723404.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c18723404.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c18723404.filter,tp,LOCATION_GRAVE,0,1,1,nil)
