...
function c30303023.initial_effect(c)
    ...
    --change effect
    local e1=Effect.CreateEffect(c)
    e1:SetDescription(aux.Stringid(30303023,0))
    e1:SetType(EFFECT_TYPE_QUICK_O)
    e1:SetCode(EVENT_CHAINING)
    e1:SetRange(LOCATION_MZONE)
    e1:SetCountLimit(1,EFFECT_COUNT_CODE_SINGLE)
    e1:SetCondition(c30303023.chcon)
    e1:SetTarget(c30303023.chtg)
    e1:SetOperation(c30303023.chop)
    c:RegisterEffect(e1)
end
function c30303023.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(30303023)
end
function c30303023.chcon(e,tp,eg,ep,ev,re,r,rp)
    local rc=re:GetHandler()
    local loc=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	if Duel.IsExistingTarget(c30303023.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
...
function c30303023.chtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(Card.IsControlerCanBeChanged,tp,0,LOCATION_MZONE,1,nil) end
end
function c30303023.chop(e,tp,eg,ep,ev,re,r,rp)
    local g=Group.CreateGroup()
    Duel.ChangeTargetCard(ev,g)
    Duel.ChangeChainOperation(ev,c30303023.repop)
end
function c30303023.filter(c)
    return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsDestructable()
end
function c30303023.repop(e,tp,eg,ep,ev,re,r,rp)
