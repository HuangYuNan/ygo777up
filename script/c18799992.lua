...
function c18799992.thfilter(c)
	return c:IsFaceup() and c:IsCanTurnSet() and not c:IsStatus(STATUS_LEAVE_CONFIRMED) and c:GetSequence()~=6 and c:GetSequence()~=7 and c:GetSequence()~=5
end
function c18799992.retg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_SZONE+LOCATION_MZONE) and c18799992.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c18799992.thfilter,tp,LOCATION_SZONE+LOCATION_MZONE,LOCATION_SZONE+LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,c18799992.thfilter,tp,LOCATION_SZONE+LOCATION_MZONE,LOCATION_SZONE+LOCATION_MZONE,1,1,nil)
