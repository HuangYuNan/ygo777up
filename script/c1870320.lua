--偶操少女 傀儡公主
function c1870320.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,nil,4,2)
	c:EnableReviveLimit()
	--pos
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(1870320,0))
	...
	e2:SetCountLimit(1)
	e2:SetCost(c1870320.cost)
	e2:SetTarget(c1870320.postg)
	e2:SetOperation(c1870320.posop)
	c:RegisterEffect(e2)
	--equip
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(1870320,1))
	e1:SetCategory(CATEGORY_EQUIP)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetCost(c1870320.eqcost)
	e1:SetTarget(c1870320.eqtg)
	e1:SetOperation(c1870320.eqop)
	c:RegisterEffect(e1)
end
function c1870320.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c1870320.postg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c1870320.eqop(e,tp,eg,ep,ev,re,r,rp)
