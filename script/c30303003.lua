...
function c30303003.thfilter(c)
    return c:IsSetCard(0xabb) and c:IsAbleToRemoveAsCost()
end
function c30303003.cost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c30303003.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
    local g=Duel.SelectMatchingCard(tp,c30303003.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
