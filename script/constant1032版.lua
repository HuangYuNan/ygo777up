...
TYPE_QUICKPLAY		=0x10000	--速攻魔法卡
TYPE_CONTINUOUS		=0x20000	--永续魔法/陷阱卡
TYPE_EQUIP			=0x40000	--装备魔法卡
TYPE_FIELD			=0x80000	--场地魔法卡
TYPE_COUNTER		=0x100000	--反击陷阱卡
TYPE_FLIP			=0x200000	--翻转怪兽
TYPE_TOON			=0x400000	--卡通怪兽
TYPE_XYZ			=0x800000	--超量怪兽
--Attributes 属性
...
REASON_MATERIAL		=0x8		--作为融合/同调/超量素材
...
STATUS_UNSUMMONABLE_CARD	=0x80000	--不能召唤
...
EFFECT_FLAG_MULTIACT_HAND	=0x0200		--手牌有多张可以同时诱发（一般不可以，比如2张龙虾只能召一张，但是永火恶魔有调整特例）
...
EFFECT_DISABLE					=2		--效果无效
EFFECT_CANNOT_DISABLE			=3		--效果不能无效
EFFECT_SET_CONTROL				=4		--改变控制器
EFFECT_CANNOT_CHANGE_CONTROL	=5		--不能改变控制权
EFFECT_CANNOT_ACTIVATE			=6		--不能发动
EFFECT_CANNOT_TRIGGER			=7		--不能触发
EFFECT_DISABLE_EFFECT			=8		--效果无效 （和EFFECT_DISABLE的区别不明）
...
EFFECT_EXTRA_SUMMON_COUNT		=29		--增加通常召唤次数
...
EFFECT_UPDATE_DEFENSE			=104	--改变防御力（参考攻击力，下同）
EFFECT_SET_DEFENSE				=105	--设置防御力
EFFECT_SET_DEFENSE_FINAL		=106	--设置最终攻击力
EFFECT_SET_BASE_DEFENSE			=107	--设置原本攻击力
...
EFFECT_MUST_BE_ATTACKED			=195	--必须攻击这张卡
...
--Global flag	--特殊标记
GLOBALFLAG_DECK_REVERSE_CHECK	=0x1		--检查卡组是否翻转
GLOBALFLAG_BRAINWASHING_CHECK	=0x2		--检查是否洗脑解除
GLOBALFLAG_SCRAP_CHIMERA		=0x4		--检查废铁奇美拉
...
DUEL_SIMPLE_AI			=0x40		--AI（不发动不舒服斯基）
