--魔人少女 拜伊科
function c1876031.initial_effect(c)
	--synchro summon
	...
	e1:SetOperation(c1876031.disop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	--e4:SetValue(c1876031.efilter)
	--c:RegisterEffect(e4)
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_SINGLE)
	e5:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e5:SetValue(c1876031.ind2)
	c:RegisterEffect(e5)
	--disable and destroy
	...
	--e6:SetOperation(c1876031.disop2)
	--c:RegisterEffect(e6)
end
function c1876031.ind2(e,c)
...
function c1876031.efilter(e,re)
...
function c1876031.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(1876031)
end
function c1876031.disop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsExistingTarget(c1876031.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
	...
	if ep==tp or Duel.GetFieldGroup(tp,LOCATION_ONFIELD,0):Filter(c1876031.Fkfilter,nil):GetCount()~=Duel.GetFieldGroup(1-tp,LOCATION_ONFIELD,0):Filter(c1876031.Fkfilter,nil):GetCount() then return end
	end
	Duel.Hint(HINT_CARD,0,1876031)
	Duel.NegateSummon(eg)
	Duel.Destroy(eg,REASON_EFFECT)
end
function c1876031.Fkfilter(c)
