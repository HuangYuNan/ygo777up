...
function c5012622.sumop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x350))
	e1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1,tp)
	Duel.RegisterFlagEffect(tp,5012622,RESET_PHASE+PHASE_END,0,1)
end
function c5012622.filter(c)
	return c:IsAbleToDeck() and (c:IsLocation(LOCATION_EXTRA) or c:IsLocation(LOCATION_GRAVE) or c:IsLocation(LOCATION_REMOVED) and c:IsFaceup()) and c:IsSetCard(0x350) 
end
