--藏书屋·八百万巫女
function c60016.initial_effect(c)
	--synchro summon
	...
	e3:SetCountLimit(1,60016)
	e3:SetTarget(c60016.thtg)
	e3:SetOperation(c60016.thop)
	c:RegisterEffect(e3)
end
function c60016.thfilter(c)
	return c:IsSetCard(0x300) and c:IsType(TYPE_MONSTER) and not c:IsCode(60016) and c:IsAbleToHand()
end
function c60016.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsDestructable,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(c60016.thfilter,tp,LOCATION_DECK,0,1,nil) end
	local g=Duel.GetMatchingGroup(c60016.thfilter,tp,LOCATION_DECK,0,nil)
...
function c60016.thop(e,tp,eg,ep,ev,re,r,rp)
