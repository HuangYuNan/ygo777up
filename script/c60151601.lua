...
function c60151601.initial_effect(c)
    ...
    e3:SetDescription(aux.Stringid(60151601,2))
    e3:SetCategory(CATEGORY_TOHAND)
    e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
    e3:SetCode(EVENT_SUMMON_SUCCESS)
    e3:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
    e3:SetCountLimit(1,60151601)
    e3:SetTarget(c60151601.sptg)
    e3:SetOperation(c60151601.spop)
    c:RegisterEffect(e3)
	local e4=e3:Clone()
    e4:SetCode(EVENT_SPSUMMON_SUCCESS)
	e4:SetCondition(c60151601.spcondition)
    c:RegisterEffect(e4)
end
...
function c60151601.filter3(c)
    return c:IsSetCard(0xc204) and c:IsType(TYPE_PENDULUM) and c:IsAbleToHand()
end
