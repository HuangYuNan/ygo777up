...
function c3033034.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFacedown() or not tc:IsRelateToEffect(e) or tc:IsControler(1-tp) or tc:IsImmuneToEffect(e) or not Duel.IsExistingMatchingCard(c3033034.cfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) then return end
	...
	if op==0 then
		...
		if sg:GetCount()>0 then
			...
			while tc do
				...
				e1:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
				tc:RegisterEffect(e1)
				tc:RegisterFlagEffect(3033034,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
				tc=sg:GetNext()
			end
		end
