...
function c18730335.initial_effect(c)
	...
	e3:SetCost(c18730335.cost)
	e3:SetTarget(c18730335.rectg)
	e3:SetOperation(c18730335.recop)
	c:RegisterEffect(e3)
end
function c18730335.atkval(e,c)
	local lps=Duel.GetLP(c:GetControler())
	local lpo=Duel.GetLP(1-c:GetControler())
	if lps>=lpo then return 0
	else return lpo-lps end
end
function c18730335.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLPCost(tp,1500)
	else Duel.PayLPCost(tp,1500) end
end
function c18730335.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c18730335.recop(e,tp,eg,ep,ev,re,r,rp)
