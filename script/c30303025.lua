...
function c30303025.initial_effect(c)
    ...
    e2:SetTarget(c30303025.damtg)
    e2:SetOperation(c30303025.damop)
    c:RegisterEffect(e2)
    local e3=e2:Clone()
    e3:SetCode(EVENT_BE_BATTLE_TARGET)
    c:RegisterEffect(e3)
end
function c30303025.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(30303025)
end
function c30303025.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return true end
    Duel.SetTargetPlayer(1-tp)
    Duel.SetTargetParam(1200)
    Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1200)
end
function c30303025.damop(e,tp,eg,ep,ev,re,r,rp)
    local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
    Duel.Damage(p,d,REASON_EFFECT)
		if Duel.IsExistingTarget(c30303025.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
		Duel.Damage(p,d,REASON_EFFECT)
		end
end
