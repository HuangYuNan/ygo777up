--少女召来的少女
function c1870300.initial_effect(c)
	--pendulum summon
	...
	e2:SetCountLimit(1,1870300)
	e2:SetCondition(c1870300.spcon)
	e2:SetTarget(c1870300.sptg)
	e2:SetOperation(c1870300.spop)
	c:RegisterEffect(e2)
	--spsummon
	...
	e3:SetCountLimit(1,18703000)
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	e3:SetTarget(c1870300.tg)
	e3:SetOperation(c1870300.op)
	c:RegisterEffect(e3)
end
function c1870300.cfilter(c,tp)
...
function c1870300.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c1870300.cfilter,1,nil,tp)
end
function c1870300.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c1870300.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c1870300.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		...
		local result=g:FilterCount(c1870300.filter2,nil)>0
		return result
	end
	Duel.SetTargetPlayer(tp)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,LOCATION_DECK)
end
function c1870300.filter2(c,tp)
	return not c:IsCode(TYPE_PENDULUM)
end
function c1870300.filter(c,tp)
	return c:IsType(TYPE_PENDULUM) and c:IsSetCard(0xabb)
end
function c1870300.op(e,tp,eg,ep,ev,re,r,rp)
	local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
	Duel.ConfirmDecktop(p,4)
	local g=Duel.GetDecktopGroup(p,4)
	local g2=g:Filter(c1870300.filter,nil)
