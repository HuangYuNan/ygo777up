--读心妖怪 古明地觉
function c45712530.initial_effect(c)
    --pendulum summon
    aux.EnablePendulumAttribute(c)
	--guess1
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(45712530,2))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetCountLimit(1,45712530)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCondition(c45712530.con1)
	e1:SetTarget(c45712530.target)
	e1:SetOperation(c45712530.op1)
	c:RegisterEffect(e1)
	--guess
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(45712530,1))
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetCountLimit(1)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c45712530.con)
	e3:SetOperation(c45712530.op)
	c:RegisterEffect(e3)
	--tohand
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(45712530,0))
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e2:SetCode(EVENT_ATTACK_ANNOUNCE)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCondition(c45712530.condition)
	e2:SetTarget(c45712530.target2)
	e2:SetOperation(c45712530.activate)
	c:RegisterEffect(e2)
	--splimit
	...
	ed:SetTarget(c45712530.splimit)
	c:RegisterEffect(ed)
end
function c45712530.splimit(e,c,tp,sumtp,sumpos)
...
function c45712530.con1(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)~=0
end
function c45712530.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_GRAVE,1,nil) end
end
function c45712530.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc1=Duel.SelectMatchingCard(tp,Card.IsAbleToDeck,tp,0,LOCATION_GRAVE,1,1,nil)
	  if tc1:GetCount()>0 then
		Duel.BreakEffect()
		local opt=Duel.SelectOption(tp,aux.Stringid(45712530,3),aux.Stringid(45712530,4))
		Duel.SendtoDeck(tc1,nil,opt,REASON_EFFECT)
	  end
	end
end
function c45712530.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c45712530.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not e:GetHandler():IsStatus(STATUS_CHAINING) end
	Duel.SetTargetCard(Duel.GetAttacker())
end
function c45712530.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c45712530.con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)~=0
end
function c45712530.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or c:IsFacedown() or Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)==0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CARDTYPE)
	local res=Duel.SelectOption(tp,70,71,72)
