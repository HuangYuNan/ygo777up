...
function c30303013.initial_effect(c)
	...
	e3:SetTarget(c30303013.rTarget)
	e3:SetOperation(c30303013.rOperation)
	c:RegisterEffect(e3)
	--search
...
function c30303013.filter(c,e,tp,m1,m2)
...
function c30303013.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemove()
end
function c30303013.rTarget(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg1=Duel.GetRitualMaterial(tp)
		local mg2=Duel.GetMatchingGroup(c30303013.mfilter,tp,LOCATION_GRAVE,0,nil)
		return Duel.IsExistingMatchingCard(c30303013.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg1,mg2)
	end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c30303013.rOperation(e,tp,eg,ep,ev,re,r,rp)
	local seq=e:GetHandler():GetSequence()
	if seq~=6 and seq~=7 then return false end
	local mg1=Duel.GetRitualMaterial(tp)
	local mg2=Duel.GetMatchingGroup(c30303013.mfilter,tp,LOCATION_GRAVE,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tg=Duel.SelectMatchingCard(tp,c30303013.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg1,mg2)
...
function c30303013.filter3(c,e,tp)
	return c:IsCode(30303031)  and c:IsAbleToHand()
end
