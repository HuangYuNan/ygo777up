...
function c1003.initial_effect(c)
    ...
    e1:SetCondition(c1003.spcon)
	e1:SetTarget(c1003.sptg)
	e1:SetOperation(c1003.spop)
	c:RegisterEffect(e1)
	--yami
...
function c1003.spcon(e,tp,eg,ep,ev,re,r,rp)
	return  re:GetHandler():IsControler(tp) 
end
function c1003.spfilter(c,e,tp)
	return  c:IsType(TYPE_MONSTER) and c:IsSetCard(0x989)  and c:IsCanBeSpecialSummoned(e,0,tp,false,false) 
end
function c1003.tdtfilter(c)
	return c:IsAbleToDeck()
end
function c1003.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 or Duel.GetLocationCount(tp,LOCATION_SZONE)>0)
		and Duel.IsExistingMatchingCard(c1003.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) and 
		Duel.IsExistingMatchingCard(c1003.tdfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c1003.spop(e,tp,eg,ep,ev,re,r,rp)
    if not e:GetHandler():IsRelateToEffect(e) or e:GetHandler():IsFacedown() then return end
	local ft1=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local ft2=Duel.GetLocationCount(tp,LOCATION_SZONE)
	if ft1<=0 and ft2<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,c1003.tdtfilter,tp,LOCATION_HAND,0,1,1,nil)
	local hc=g:GetFirst()
	if hc  and Duel.SendtoDeck(hc,nil,2,REASON_EFFECT)>0 then
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c1003.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	if tc then
