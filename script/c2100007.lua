...
function c2100007.filter(c)
	return c:IsSetCard(0x1219) and c:IsType(TYPE_MONSTER) and c:IsFaceup()
end
...
function c2100007.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x1219)
end
...
function c2100007.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c2100007.tdfilter,tp,0,LOCATION_ONFIELD,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,c2100007.tdfilter,tp,0,LOCATION_ONFIELD,1,1,e:GetHandler())
