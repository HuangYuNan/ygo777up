--魔人少女 莎姆&莎瑪
function c1876018.initial_effect(c)
	--synchro summon
	...
	--e1:SetCondition(c1876018.remcon)
	--e1:SetCost(c1876018.cost)
	--e1:SetTarget(c1876018.remtg)
	--e1:SetOperation(c1876018.remop)
	--c:RegisterEffect(e1)
	...
	--e3:SetValue(c1876018.efilter)
	--c:RegisterEffect(e3)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e1:SetValue(c1876018.ind2)
	c:RegisterEffect(e1)
    --Disable
    ...
    e2:SetTarget(c1876018.damtg)
    e2:SetOperation(c1876018.damop)
    c:RegisterEffect(e2)
    local e3=e2:Clone()
    e3:SetCode(EVENT_BE_BATTLE_TARGET)
    c:RegisterEffect(e3)
end
function c1876018.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(1876018)
end
function c1876018.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return true end
    Duel.SetTargetPlayer(1-tp)
    Duel.SetTargetParam(1200)
    Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1200)
end
function c1876018.damop(e,tp,eg,ep,ev,re,r,rp)
    local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
    Duel.Damage(p,d,REASON_EFFECT)
		if Duel.IsExistingTarget(c1876018.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
		Duel.Damage(p,d,REASON_EFFECT)
		end
end

function c1876018.remcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c1876018.costfilter(c)
	return c:IsSetCard(0xabb) and not c:IsPublic()
end
function c1876018.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1876018.costfilter,tp,LOCATION_HAND,0,1,nil,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SelectMatchingCard(tp,c1876018.costfilter,tp,LOCATION_HAND,0,1,1,nil,nil)
	Duel.ConfirmCards(1-tp,g)
	Duel.ShuffleHand(tp)
end
function c1876018.remtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c1876018.remop(e,tp,eg,ep,ev,re,r,rp)
...
function c1876018.efilter(e,re)
