--魔人少女 撒妲
function c18706032.initial_effect(c)
	--synchro summon
	...
	e3:SetCondition(c18706032.thcon)
	e3:SetTarget(c18706032.efilter)
	e3:SetValue(c18706032.efilter2)
	c:RegisterEffect(e3)
	--disable and destroy
	...
	e6:SetCondition(c18706032.chcon)
	e6:SetOperation(c18706032.chop)
	c:RegisterEffect(e6)
end
function c18706032.chcon(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
	local loc=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	return (rc:GetType()==TYPE_SPELL or rc:GetType()==TYPE_TRAP) and re:IsHasType(EFFECT_TYPE_ACTIVATE)
end
function c18706032.chop(e,tp,eg,ep,ev,re,r,rp)
	local g=Group.CreateGroup()
	Duel.ChangeTargetCard(ev,g)
	Duel.ChangeChainOperation(ev,c18706032.repop)
end
function c18706032.filter(c)
	return c:IsDestructable()
end
function c18706032.repop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c18706032.filter,tp,0,LOCATION_ONFIELD,1,1,nil)
	if g:GetCount()>0 then
		Duel.HintSelection(g)
		Duel.Destroy(g,REASON_EFFECT)
	end
end
function c18706032.thcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c18706032.mfilter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,e:GetHandler())
end
function c18706032.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(18706032)
end
function c18706032.ind2(e,c)
...
function c18706032.efilter(e,c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d)
end
function c18706032.efilter2(e,re)
