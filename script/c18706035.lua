...
function c18706035.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c18706035.tgfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c18706035.tgfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local sg=Duel.SelectTarget(tp,c18706035.tgfilter,tp,LOCATION_GRAVE,0,1,1,nil)
...
function c18706035.xyzcon(e,c,og)
	...
	local abcount=0
	if ft>0 and Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_MZONE,nil)>0 and Duel.IsExistingMatchingCard(c18706035.xyzfilter1,c:GetControler(),LOCATION_HAND+LOCATION_EXTRA,0,1,nil,c) 
	and  Duel.IsExistingMatchingCard(c18706035.xyzfilter2,c:GetControler(),LOCATION_HAND+LOCATION_EXTRA,0,1,nil,c)  then abcount=abcount+2 end
...
function c18706035.xyzop(e,tp,eg,ep,ev,re,r,rp,c,og)
	if og then
		...
		if sel==2 then
			mg1=Duel.SelectMatchingCard(tp,c18706035.xyzfilter1,tp,LOCATION_HAND+LOCATION_EXTRA,0,1,1,nil,c)
			mg2=Duel.SelectMatchingCard(tp,c18706035.xyzfilter2,tp,LOCATION_HAND+LOCATION_EXTRA,0,1,1,nil,c)
		else
