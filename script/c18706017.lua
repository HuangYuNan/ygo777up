--魔人少女 普露芙埃托
function c18706017.initial_effect(c)
	--synchro summon
	...
	--e1:SetCondition(c18706017.remcon)
	--e1:SetCost(c18706017.cost)
	--e1:SetTarget(c18706017.remtg)
	--e1:SetOperation(c18706017.remop)
	--c:RegisterEffect(e1)
	...
	--e3:SetValue(c18706017.efilter)
	--c:RegisterEffect(e3)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e2:SetValue(c18706017.ind2)
	c:RegisterEffect(e2)
    --change effect
    local e1=Effect.CreateEffect(c)
    e1:SetDescription(aux.Stringid(18706017,0))
    e1:SetType(EFFECT_TYPE_QUICK_O)
    e1:SetCode(EVENT_CHAINING)
    e1:SetRange(LOCATION_MZONE)
    e1:SetCountLimit(1,EFFECT_COUNT_CODE_SINGLE)
    e1:SetCondition(c18706017.chcon)
    e1:SetTarget(c18706017.chtg)
    e1:SetOperation(c18706017.chop)
    c:RegisterEffect(e1)
end
function c18706017.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(18706017)
end
function c18706017.chcon(e,tp,eg,ep,ev,re,r,rp)
    local rc=re:GetHandler()
    local loc=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	if Duel.IsExistingTarget(c18706017.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
...
function c18706017.chtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(Card.IsControlerCanBeChanged,tp,0,LOCATION_MZONE,1,nil) end
end
function c18706017.chop(e,tp,eg,ep,ev,re,r,rp)
    local g=Group.CreateGroup()
    Duel.ChangeTargetCard(ev,g)
    Duel.ChangeChainOperation(ev,c18706017.repop)
end
function c18706017.filter(c)
    return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsDestructable()
end
function c18706017.repop(e,tp,eg,ep,ev,re,r,rp)
...
function c18706017.remcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c18706017.costfilter(c)
	return c:IsSetCard(0xabb) and not c:IsPublic()
end
function c18706017.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c18706017.costfilter,tp,LOCATION_HAND,0,1,nil,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SelectMatchingCard(tp,c18706017.costfilter,tp,LOCATION_HAND,0,1,1,nil,nil)
	Duel.ConfirmCards(1-tp,g)
	Duel.ShuffleHand(tp)
end
function c18706017.remtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c18706017.remop(e,tp,eg,ep,ev,re,r,rp)
...
function c18706017.efilter(e,re)
