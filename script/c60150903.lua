...
Duel.EnableGlobalFlag(GLOBALFLAG_DECK_REVERSE_CHECK)
	...
	e3:SetValue(c60150903.xyzlimit)
	c:RegisterEffect(e3)
end
...
function c60150903.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		if c:IsCanBeSpecialSummoned(e,0,tp,false,false) then
			...
			local g=Duel.GetFieldGroup(1-tp,0,LOCATION_SZONE):RandomSelect(1-tp,1)
			if Duel.Remove(g,POS_FACEUP,REASON_EFFECT)~=0 then
				local g2=Duel.GetFieldGroup(1-tp,0,LOCATION_EXTRA):RandomSelect(1-tp,1)
				local g3=Duel.GetFieldGroup(1-tp,0,LOCATION_GRAVE):RandomSelect(1-tp,1)
