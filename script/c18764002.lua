...
function c18764002.setfilter(c,tp)
	return c:IsType(TYPE_TRAP) and c:IsSSetable(true) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and c:IsSetCard(0xaabb) and not c:IsCode(18764002)
end
function c18764002.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_DECK) and c18764002.setfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c18764002.setfilter,tp,LOCATION_DECK,0,1,nil,tp) end
end
function c18764002.op(e,tp,eg,ep,ev,re,r,rp)
		if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
		local g=Duel.SelectMatchingCard(tp,c18764002.setfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c18764002.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,18764002,0xaabb,0x21,5,1700,1800,RACE_ZOMBIE,ATTRIBUTE_LIGHT) end
...
function c18764002.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,18764002,0xaabb,0x21,5,1700,1800,RACE_ZOMBIE,ATTRIBUTE_LIGHT) then
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
