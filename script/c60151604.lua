...
function c60151604.filter(c)
    return c:IsFaceup() and c:IsSetCard(0xc204) and c:IsType(TYPE_MONSTER)
end
...
function c60151604.condition(e,tp,eg,ep,ev,re,r,rp)
    ...
    return re:IsActiveType(TYPE_MONSTER) and re:IsActivated() and rc:IsSetCard(0xc204)
end
...
function c60151604.filter3(c,e,tp)
    return c:IsSetCard(0xc204) and c:IsType(TYPE_PENDULUM) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c60151604.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.SelectMatchingCard(tp,c60151604.filter3,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,e,tp)
