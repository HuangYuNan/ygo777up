...
function c18712302.initial_effect(c)
	...
	e4:SetCost(c18712302.cost)
	e4:SetTarget(c18712302.sptg)
	e4:SetOperation(c18712302.spop)
	c:RegisterEffect(e4)
end
function c18712302.mat_check(c)
    return not c:IsCode(111111)
end
function c18712302.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() end
	Duel.Release(e:GetHandler(),REASON_COST)
end
function c18712302.scfilter(c,e,tp)
	return (c:GetSequence()==6 or c:GetSequence()==7)
		and (c:IsSetCard(0xabb) or c:IsSetCard(0xca1)) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function c18712302.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and c18712302.scfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c18712302.scfilter,tp,LOCATION_SZONE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
end
function c18712302.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(c18712302.scfilter,tp,LOCATION_SZONE,0,1,nil,e,tp)
