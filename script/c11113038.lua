...
function c11113038.initial_effect(c)
	...
	e2:SetCondition(c11113038.sprcon)
	e2:SetOperation(c11113038.sprop)
	c:RegisterEffect(e2)
	--negate
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(11113038,0))
	e3:SetCategory(CATEGORY_DISABLE)
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetCode(EVENT_FREE_CHAIN)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1,11113038)
	e3:SetHintTiming(0,0x1c0)
	e3:SetCost(c11113038.discost)
	e3:SetTarget(c11113038.distg)
	e3:SetOperation(c11113038.disop)
	c:RegisterEffect(e3)
	--index
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(11113038,1))
	e4:SetType(EFFECT_TYPE_QUICK_O)
	e4:SetCode(EVENT_FREE_CHAIN)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetRange(LOCATION_GRAVE)
	e4:SetCountLimit(1,111130380)
	e4:SetCost(c11113038.indcost)
	e4:SetTarget(c11113038.indtg)
	e4:SetOperation(c11113038.indop)
	c:RegisterEffect(e4)
end
function c11113038.ffilter(c)
	return c:IsFusionSetCard(0x15c) and bit.band(c:GetType(),0x81)==0x81
end
function c11113038.spfilter(c)
	return c:IsFusionSetCard(0x15c) and bit.band(c:GetType(),0x81)==0x81 and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial()
end
function c11113038.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2
		and Duel.IsExistingMatchingCard(c11113038.spfilter,tp,LOCATION_MZONE,0,2,nil)
end
function c11113038.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c11113038.spfilter,tp,LOCATION_MZONE,0,2,2,nil)
	c:SetMaterial(g)
