...
function c187187001.initial_effect(c)
	...
	e4:SetCountLimit(1,187187001)
	e4:SetCondition(c187187001.condition)
	--e4:SetCost(c187187001.cost)
	e4:SetTarget(c187187001.target)
	e4:SetOperation(c187187001.activate)
	c:RegisterEffect(e4)
end
function c187187001.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c187187001.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c187187001.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	local tc=g:GetFirst()
...
function c187187001.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:GetHandler():IsRelateToEffect(re) then
		...
		local g2=Duel.SelectTarget(tp,c187187001.ccfilter2,tp,LOCATION_MZONE,0,1,1,nil)
...
function c187187001.ccfilter2(c)
