--妮普顿的三叉戟
function c30303040.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCountLimit(1,30303040)
	e1:SetCost(c30303040.hspcost)
	e1:SetTarget(c30303040.target)
	e1:SetOperation(c30303040.activate)
	c:RegisterEffect(e1)
end
function c30303040.dfilter(c)
	return c:IsAbleToGraveAsCost() and c:IsSetCard(0xabb)
end
function c30303040.hspcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c30303040.dfilter,tp,LOCATION_REMOVED,0,3,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c30303040.dfilter,tp,LOCATION_REMOVED,0,3,3,nil)
	Duel.SendtoGrave(g,REASON_COST+REASON_RETURN)
end
function c30303040.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c30303040.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SelectOption(1-tp,aux.Stringid(30303040,0))
	Duel.SelectOption(tp,aux.Stringid(30303040,0))
		...
		Duel.Hint(HINT_CARD,0,30303046)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
