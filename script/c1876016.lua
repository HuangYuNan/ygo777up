--魔人少女 梅妮丝华兹
function c1876016.initial_effect(c)
	--synchro summon
	...
	--e1:SetCondition(c1876016.remcon)
	--e1:SetCost(c1876016.cost)
	--e1:SetTarget(c1876016.remtg)
	--e1:SetOperation(c1876016.remop)
	--c:RegisterEffect(e1)
	...
	--e3:SetValue(c1876016.efilter)
	--c:RegisterEffect(e3)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e2:SetValue(c1876016.ind2)
	c:RegisterEffect(e2)
	--negate
	local e3=Effect.CreateEffect(c)
	...
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c1876016.discon)
	e3:SetCost(c1876016.discost)
	e3:SetTarget(c1876016.distg)
	e3:SetOperation(c1876016.disop)
	c:RegisterEffect(e3)
end
function c1876016.discon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev) and rp~=tp
end
function c1876016.Rfilter(c,code)
	return c:GetCode()==code and c:IsAbleToRemove()
end
function c1876016.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(1876016)
end
function c1876016.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	if Duel.IsExistingTarget(c1876016.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
	if chk==0 then return Duel.IsExistingMatchingCard(c1876016.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,LOCATION_GRAVE,1,nil,eg:GetFirst():GetCode()) end
	local g=Duel.SelectMatchingCard(tp,c1876016.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,LOCATION_GRAVE,1,1,nil,eg:GetFirst():GetCode())
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	else
	if chk==0 then return Duel.IsExistingMatchingCard(c1876016.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,nil,eg:GetFirst():GetCode()) end
	local g=Duel.SelectMatchingCard(tp,c1876016.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,eg:GetFirst():GetCode())
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	end
end
function c1876016.distg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c1876016.disop(e,tp,eg,ep,ev,re,r,rp)
	Duel.NegateActivation(ev)
	if re:GetHandler():IsRelateToEffect(re) then
		Duel.Destroy(eg,REASON_EFFECT)
	end
end
function c1876016.ind2(e,c)
...
function c1876016.efilter(e,re)
