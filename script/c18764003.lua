...
function c18764003.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c18764003.setfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c18764003.setfilter,tp,LOCATION_GRAVE,0,1,nil,tp) end
end
function c18764003.spop(e,tp,eg,ep,ev,re,r,rp)
		if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
		local g=Duel.SelectMatchingCard(tp,c18764003.setfilter,tp,LOCATION_GRAVE,0,1,1,nil)
