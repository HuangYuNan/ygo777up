...
function c30303022.initial_effect(c)
	...
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c30303022.discon)
	e3:SetCost(c30303022.discost)
	e3:SetTarget(c30303022.distg)
	e3:SetOperation(c30303022.disop)
	c:RegisterEffect(e3)
end
function c30303022.discon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev) and rp~=tp
end
function c30303022.Rfilter(c,code)
	return c:GetCode()==code and c:IsAbleToRemove()
end
function c30303022.mfilter(c)
	return c:IsSetCard(0xabb) and c:IsSetCard(0x6d) and not c:IsCode(30303022)
end
function c30303022.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	if Duel.IsExistingTarget(c30303022.mfilter,tp,LOCATION_MZONE,0,1,nil) then 
	if chk==0 then return Duel.IsExistingMatchingCard(c30303022.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,LOCATION_GRAVE,1,nil,eg:GetFirst():GetCode()) end
	local g=Duel.SelectMatchingCard(tp,c30303022.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,LOCATION_GRAVE,1,1,nil,eg:GetFirst():GetCode())
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	else
	if chk==0 then return Duel.IsExistingMatchingCard(c30303022.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,nil,eg:GetFirst():GetCode()) end
	local g=Duel.SelectMatchingCard(tp,c30303022.Rfilter,tp,LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,eg:GetFirst():GetCode())
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	end
end
function c30303022.distg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c30303022.disop(e,tp,eg,ep,ev,re,r,rp)
