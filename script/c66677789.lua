...
function c66677789.initial_effect(c)
	...
	e4:SetTarget(function(e,tp,eg,ep,ev,re,r,rp,chk)
		local s0=Duel.IsExistingMatchingCard(c66677789.tfilter,tp,LOCATION_GRAVE,0,2,nil,0)
		local s1=Duel.IsExistingMatchingCard(c66677789.tfilter,tp,LOCATION_REMOVED,0,2,nil,1)
...
function c66677789.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (exm and c:GetFlagEffect(66677789)<2 and e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) and Duel.IsExistingMatchingCard(c66677789.costfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,nil)) or c:GetFlagEffect(66677789)==0 end
	if c:GetFlagEffect(66677789)==0 then
		e:SetLabel(1)
	else
		e:SetLabel(0)
		e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
		local g=Duel.SelectMatchingCard(tp,c66677789.costfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil)
